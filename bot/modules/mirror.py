...
from bot.helper.ext_utils.bot_utils import is_url, is_magnet, is_gdtot_link, is_appdrive_link, is_mega_link, is_gdrive_link, get_content_type
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0, qbsd=False):
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            ...
            Thread(target=add_gd_download, args=(link, listener, is_gdtot or is_appdrive, name)).start()
